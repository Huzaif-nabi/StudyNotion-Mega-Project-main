...
const Navbar = () => {
    ...
    const matchRoute = (route) => {
        // Handle cases where route might be undefined
                                        ...
                                        <div className={`invisible absolute left-[50%] 
                                            ...
                                            {loading ? (
                                                <span className="loader"></span>
                                                 ...
                                                 // Check if subLinks is an array and has items
                                                subLinks?.length > 0 ? (
                                                    subLinks.map((subLink) => (
                                                        <Link
                                                            className='rounded-lg bg-transparent py-4 pl-4 hover:bg-richblack-50'
                                                            to={`/catalog/${subLink.name.toLowerCase().split(" ").join("-")}`} // Create a URL-friendly slug
                                                            key={subLink._id} // Use a unique ID for the key
                                                        >
                                                            <p>{subLink.name}</p>
                                                        </Link>
                                                    ))
